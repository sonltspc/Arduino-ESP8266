...
esp8266_nsp.on('connection', function(socket) {
  ...
  // Khi esp8266 kết nối với Server thì gủi OK đến ESP.
    esp8266_nsp.emit("OK","")

  // Khi esp8266 bị mất kết nối
	socket.on('disconnect', function() {
		console.log("Disconnect socket esp8266")
    // Khi esp8266 mất kết nối thì gửi NO về Webapp
    webapp_nsp.emit("NO","")
	})

	//nhận được bất cứ lệnh nào từ esp8266
  ...
  // Khi Server nhận REFRESH thì gửi REFRESH về esp8266
  socket.on("REFRESH", function(packet) {
    esp8266_nsp.emit("REFRESH","")
  })
...
//nhận bất cứ lệnh nào từ webapp
	 socket.on('*', function(packet) {
